...
// UserSignup 用户注册
func UserSignup(email, password, nickname string,stu_id string,stu_name,stu_photo string) (*model.User, error) {
	newUser := &model.User{
		Email: email,
	}
	// 生成密码
	...
	newUser.StuVaild = false
	newUser.StuId = stu_id
	newUser.StuName = stu_name
	newUser.StuPhoto = stu_photo
	// 查看是否为超级用户
